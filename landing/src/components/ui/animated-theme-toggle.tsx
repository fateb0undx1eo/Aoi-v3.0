...
function Button({
  ...
  return (
    <button
      className={cn(
        "inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-0 focus-visible:ring-offset-0 disabled:pointer-events-none disabled:opacity-50",
        className
      )}
      disabled={disabled}
      onClick={onClick}
...
export const AnimatedThemeToggle = ({
  ...
  if (!mounted) {
    return (
      <Button className={cn("h-9 w-9 rounded-xl", className)} variant="ghost" disabled>
  ...
  return (
    <Button
      onClick={toggleTheme}
      className={cn(
        "h-9 w-9 rounded-xl border-0 bg-transparent text-foreground shadow-none hover:bg-black/5 dark:hover:bg-white/10",
        className
      )}
      variant="ghost"

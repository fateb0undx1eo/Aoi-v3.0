...
import { Trans, useTranslation } from "@/lib/react-i18next";
...
> = {
  ...
  render(
    capture: Capture,
    render: Renderer,
    data: { [Key in keyof Data]?: Resolutions[Data[Key]] | null },
    t?: TFunction,
  ): Renderable;
};
      ...
      <span className={actionableMentionStyle}>
        {
          data.channel?.type === "post" ? (
            ...
            <>
              {channelIcons[data.channel.type as keyof typeof channelIcons]()}
              {data.channel.name ?? (
                <span className="italic">
                  <Trans t={t} i18nKey="mention.unknown" />
                </span>

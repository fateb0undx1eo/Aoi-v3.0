import { deleteWhere, fetchMany, fetchOne, updateWhere, upsertRows } from '../database/repository.js';

interface CaseCommandConfig {
  channel_id: string | null;
  allowed_role_ids: string[];
  default_timeout_minutes: number;
}

interface ModuleConfigRow {
  guild_id: string;
  module_name: string;
  enabled: boolean;
  config: Record<string, any>;
  updated_at: string;
}

interface ModConfig {
  guild_id: string;
  modlog_channel_id: string | null;
  mute_role_id: string | null;
  warn_auto_punish_enabled: boolean;
  warn_threshold_1: number;
  warn_action_1: string;
  warn_duration_1: number | null;
  warn_threshold_2: number;
  warn_action_2: string;
  warn_duration_2: number | null;
  warn_threshold_3: number;
  warn_action_3: string;
  warn_duration_3: number | null;
  dm_on_punish: boolean;
  show_mod_in_dm: boolean;
}

interface CreateCaseParams {
  guildId: string;
  targetUserId: string;
  targetUsername?: string;
  moderatorUserId: string;
  moderatorUsername?: string;
  type: string;
  reason?: string;
  durationSeconds?: number | null;
}

interface ListCaseFilters {
  userId?: string;
  type?: string;
  active?: boolean;
  limit?: number;
  offset?: number;
}

interface RevokePunishmentParams {
  guildId: string;
  caseId: number | string;
  moderatorUserId: string;
  reason?: string;
}

interface LoaPayload {
  reason?: string;
  rolePriority?: number;
  lastSeenAt?: string;
  active?: boolean;
}

interface ConfigServiceLike {
  getModuleConfig(guildId: string, moduleName: string): Promise<ModuleConfigRow>;
  upsertModuleConfig(data: Record<string, any>): Promise<void>;
}

export class ModerationService {
  private configService: ConfigServiceLike;

  constructor(configService: ConfigServiceLike) {
    this.configService = configService;
  }

  normalizeCaseCommandConfig(rawConfig: Record<string, any> = {}): CaseCommandConfig {
    const channelId = String(rawConfig?.channel_id ?? '').trim();
    const defaultTimeoutMinutes = Math.max(1, Math.min(10080, Number(rawConfig?.default_timeout_minutes) || 10));
    const normalizeIds = (values: any): string[] => (
      Array.isArray(values)
        ? values
        : String(values ?? '')
            .split(',')
            .map((value: string) => value.trim())
    ).filter(Boolean);

    return {
      channel_id: channelId || null,
      allowed_role_ids: Array.from(new Set(normalizeIds(rawConfig?.allowed_role_ids))),
      default_timeout_minutes: defaultTimeoutMinutes
    };
  }

  async getCaseCommandConfig(guildId: string): Promise<CaseCommandConfig> {
    const row = await this.configService.getModuleConfig(guildId, 'moderation').catch(() => null);
    return this.normalizeCaseCommandConfig((row as ModuleConfigRow)?.config?.case_command);
  }

  async updateCaseCommandConfig(guildId: string, updates: Record<string, any> = {}): Promise<CaseCommandConfig> {
    const row = await this.configService.getModuleConfig(guildId, 'moderation').catch(() => null);
    const currentConfig = this.normalizeCaseCommandConfig((row as ModuleConfigRow)?.config?.case_command);
    const nextConfig = this.normalizeCaseCommandConfig({
      ...currentConfig,
      ...updates
    });

    await this.configService.upsertModuleConfig({
      guild_id: guildId,
      module_name: 'moderation',
      enabled: (row as ModuleConfigRow)?.enabled ?? true,
      config: {
        ...((row as ModuleConfigRow)?.config ?? {}),
        case_command: nextConfig
      }
    });

    return nextConfig;
  }

  async setAfk(guildId: string, userId: string, reason: string): Promise<void> {
    return upsertRows('afk_states', {
      guild_id: guildId,
      user_id: userId,
      reason,
      since_at: new Date().toISOString()
    }, 'guild_id,user_id');
  }

  async clearAfk(guildId: string, userId: string): Promise<void> {
    return deleteWhere('afk_states', (table: any) => table.eq('guild_id', guildId).eq('user_id', userId));
  }

  async getAfk(guildId: string, userId: string): Promise<Record<string, any> | null> {
    const rows = await fetchMany('afk_states', (table: any) =>
      table.select('*').eq('guild_id', guildId).eq('user_id', userId)
    );
    return rows[0] ?? null;
  }

  async recordGhostPing(payload: Record<string, any>): Promise<void> {
    return upsertRows('ghost_ping_events', payload);
  }

  async getModConfig(guildId: string): Promise<ModConfig> {
    try {
      const config = await fetchOne('mod_config', (table: any) =>
        table.select('*').eq('guild_id', guildId)
      );
      return (config as ModConfig) || this.getDefaultModConfig(guildId);
    } catch {
      return this.getDefaultModConfig(guildId);
    }
  }

  async updateModConfig(guildId: string, updates: Record<string, any>): Promise<void> {
    return upsertRows('mod_config', {
      guild_id: guildId,
      ...updates,
      updated_at: new Date().toISOString()
    }, 'guild_id');
  }

  getDefaultModConfig(guildId: string): ModConfig {
    return {
      guild_id: guildId,
      modlog_channel_id: null,
      mute_role_id: null,
      warn_auto_punish_enabled: false,
      warn_threshold_1: 3,
      warn_action_1: 'MUTE',
      warn_duration_1: 3600,
      warn_threshold_2: 5,
      warn_action_2: 'KICK',
      warn_duration_2: null,
      warn_threshold_3: 7,
      warn_action_3: 'BAN',
      warn_duration_3: null,
      dm_on_punish: true,
      show_mod_in_dm: false
    };
  }

  async getNextCaseNumber(guildId: string): Promise<number> {
    try {
      const result = await fetchOne('mod_cases', (table: any) =>
        table.select('case_number')
          .eq('guild_id', guildId)
          .order('case_number', { ascending: false })
          .limit(1)
      );
      return ((result as any)?.case_number || 0) + 1;
    } catch {
      return 1;
    }
  }

  async createCase({ guildId, targetUserId, targetUsername, moderatorUserId, moderatorUsername, type, reason, durationSeconds }: CreateCaseParams): Promise<Record<string, any>> {
    const caseNumber = await this.getNextCaseNumber(guildId);

    let expiresAt: string | null = null;
    if (durationSeconds && ['TEMPBAN', 'MUTE', 'TIMEOUT'].includes(type)) {
      expiresAt = new Date(Date.now() + durationSeconds * 1000).toISOString();
    }

    const caseData: Record<string, any> = {
      guild_id: guildId,
      case_number: caseNumber,
      target_user_id: targetUserId,
      target_username: targetUsername || 'Unknown',
      moderator_user_id: moderatorUserId,
      moderator_username: moderatorUsername || 'Unknown',
      type,
      reason: reason || 'No reason provided',
      duration_seconds: durationSeconds || null,
      expires_at: expiresAt,
      active: ['TEMPBAN', 'MUTE', 'TIMEOUT', 'BAN'].includes(type),
      created_at: new Date().toISOString()
    };

    await upsertRows('mod_cases', caseData);

    if (type === 'WARN') {
      await this.checkAutoPunish(guildId, targetUserId, moderatorUserId);
    }

    return { ...caseData, id: caseNumber };
  }

  async listCases(guildId: string, { userId, type, active, limit = 50, offset = 0 }: ListCaseFilters): Promise<Record<string, any>[]> {
    return fetchMany('mod_cases', (table: any) => {
      let q = table.select('*').eq('guild_id', guildId);
      if (userId) q = q.eq('target_user_id', userId);
      if (type) q = q.eq('type', type);
      if (active !== undefined) q = q.eq('active', active);
      return q.order('created_at', { ascending: false }).limit(limit).range(offset, offset + limit - 1);
    });
  }

  async updateCase(guildId: string, caseId: number | string, updates: Record<string, any>): Promise<void> {
    return updateWhere(
      'mod_cases',
      {
        ...updates,
        updated_at: new Date().toISOString()
      },
      (table: any) => table.eq('guild_id', guildId).eq('id', caseId)
    );
  }

  async deleteCase(guildId: string, caseId: number | string): Promise<void> {
    return deleteWhere('mod_cases', (table: any) =>
      table.eq('guild_id', guildId).eq('id', caseId)
    );
  }

  async getActivePunishments(guildId: string): Promise<Record<string, any>[]> {
    return fetchMany('mod_cases', (table: any) =>
      table.select('*')
        .eq('guild_id', guildId)
        .eq('active', true)
        .in('type', ['BAN', 'TEMPBAN', 'MUTE', 'TIMEOUT'])
        .order('created_at', { ascending: false })
    );
  }

  async revokePunishment({ guildId, caseId, moderatorUserId, reason }: RevokePunishmentParams): Promise<{ ok: boolean }> {
    const existingCase = await fetchOne('mod_cases', (table: any) =>
      table.select('*').eq('guild_id', guildId).eq('id', caseId)
    );

    if (!existingCase) throw new Error('Case not found');

    const existing = existingCase as Record<string, any>;
    const unType = existing.type === 'BAN' || existing.type === 'TEMPBAN' ? 'UNBAN' : 'UNMUTE';
    await this.createCase({
      guildId,
      targetUserId: existing.target_user_id,
      targetUsername: existing.target_username,
      moderatorUserId,
      moderatorUsername: undefined,
      type: unType,
      reason: reason || `Revoked case #${existing.case_number}`
    });

    await this.updateCase(guildId, caseId, { active: false });
    return { ok: true };
  }

  async getWarnCount(guildId: string, userId: string): Promise<number> {
    const rows = await fetchMany('mod_cases', (table: any) =>
      table.select('id')
        .eq('guild_id', guildId)
        .eq('target_user_id', userId)
        .eq('type', 'WARN')
    );
    return rows.length || 0;
  }

  async clearWarns(guildId: string, userId: string, moderatorUserId: string): Promise<{ cleared: number }> {
    const warns = await fetchMany('mod_cases', (table: any) =>
      table.select('id')
        .eq('guild_id', guildId)
        .eq('target_user_id', userId)
        .eq('type', 'WARN')
    );

    await Promise.allSettled(
      warns.map((warn) => this.updateCase(guildId, (warn as any).id, { active: false }))
    );

    await this.createCase({
      guildId,
      targetUserId: userId,
      moderatorUserId,
      type: 'NOTE',
      reason: `Cleared ${warns.length} warning(s)`
    });

    return { cleared: warns.length };
  }

  async checkAutoPunish(guildId: string, userId: string, moderatorUserId: string): Promise<void> {
    const config = await this.getModConfig(guildId);
    if (!config.warn_auto_punish_enabled) return;

    const warnCount = await this.getWarnCount(guildId, userId);

    const thresholds = [
      { threshold: config.warn_threshold_3, action: config.warn_action_3, duration: config.warn_duration_3 },
      { threshold: config.warn_threshold_2, action: config.warn_action_2, duration: config.warn_duration_2 },
      { threshold: config.warn_threshold_1, action: config.warn_action_1, duration: config.warn_duration_1 }
    ];

    for (const { threshold, action, duration } of thresholds) {
      if (warnCount >= threshold && action) {
        await this.createCase({
          guildId,
          targetUserId: userId,
          moderatorUserId,
          type: action,
          reason: `Auto-punish: Reached ${warnCount} warnings`,
          durationSeconds: duration
        });
        break;
      }
    }
  }

  async setGhostPingWindow(guildId: string, seconds: number): Promise<void> {
    const row = await this.configService.getModuleConfig(guildId, 'moderation').catch(() => null);
    const config: Record<string, any> = (row as ModuleConfigRow)?.config ?? {};
    config.ghost_ping_window_seconds = seconds;
    await this.configService.upsertModuleConfig({
      guild_id: guildId,
      module_name: 'moderation',
      enabled: (row as ModuleConfigRow)?.enabled ?? true,
      config
    });
  }

  async setPingProtectionRoles(guildId: string, roleIds: string[]): Promise<void> {
    const row = await this.configService.getModuleConfig(guildId, 'moderation').catch(() => null);
    const config: Record<string, any> = (row as ModuleConfigRow)?.config ?? {};
    config.ping_protection_roles = roleIds;
    await this.configService.upsertModuleConfig({
      guild_id: guildId,
      module_name: 'moderation',
      enabled: (row as ModuleConfigRow)?.enabled ?? true,
      config
    });
  }

  async setLoa(guildId: string, userId: string, payload: LoaPayload): Promise<void> {
    return upsertRows('loa_states', {
      guild_id: guildId,
      user_id: userId,
      reason: payload.reason,
      role_priority: payload.rolePriority ?? 0,
      last_seen_at: payload.lastSeenAt ?? new Date().toISOString(),
      active: payload.active ?? true
    }, 'guild_id,user_id');
  }

  async listLoa(guildId: string): Promise<Record<string, any>[]> {
    return fetchMany('loa_states', (table: any) =>
      table
        .select('*')
        .eq('guild_id', guildId)
        .eq('active', true)
        .order('role_priority', { ascending: false })
        .order('updated_at', { ascending: false })
    );
  }

  async touchLoaLastSeen(guildId: string, userId: string): Promise<void> {
    return updateWhere('loa_states', {
      last_seen_at: new Date().toISOString()
    }, (table: any) => table.eq('guild_id', guildId).eq('user_id', userId));
  }
}

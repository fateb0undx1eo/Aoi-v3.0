...
function checkUserPermissions(member, guild) {
...
function buildProgressText({ channelName, scanned, deleted, failed, wouldDelete, elapsed, mode, targetCount, status }) {
  const lines = [
...
async function runPurge({ channel, count, dryRun, interaction }) {
  ...
  try {
    while (!done) {
      ...
      // Process messages newest-first (no sorting needed)
      for (const message of batch.values()) {
        ...
        // Use REST API directly — message.delete() hangs indefinitely
        // in some discord.js v14 setups due to the internal request queue.
        try {
          ...
          if (deleteError.code === 10008) {
            // Message already deleted - not a failure
            deleted++;
      ...
      // Advance pagination cursor to oldest message in batch
      ...
      // If batch is smaller than 100, we've reached the start of the channel
      if (batch.size < 100) {

...
function convertMarkdown(text: string): string {
  ...
  text = text.replace(/\x00UL(\d+)\x00/g, (_: string, i: string) => lists[parseInt(i)] || '');
  text = text.replace(/\x00OL(\d+)\x00/g, (_: string, i: string) => orderedLists[parseInt(i)] || '');
  text = text.replace(/\x00CB(\d+)\x00/g, (_: string, i: string) => codeBlocks[parseInt(i)] || '');
  text = text.replace(/\x00IC(\d+)\x00/g, (_: string, i: string) => inlineCodes[parseInt(i)] || '');
  return text;
}
...
function renderContent(content: string, attachments: any[], embeds: any[], stickers: any[], userMap: Record<string, string>): string {
  let html = '';

  if (content) {
    let processed = escapeHtml(content);
    processed = processed.replace(/&lt;(a?):(\w+):(\d+)&gt;/g, (_: string, a: string, name: string, id: string) =>
      `<img class="emoji" src="https://cdn.discordapp.com/emojis/${id}.${a ? 'gif' : 'png'}?size=4096" alt=":${name}:" loading="lazy">`
    );
    processed = processed.replace(/&lt;@!?(\d+)&gt;/g, (_: string, id: string) => {

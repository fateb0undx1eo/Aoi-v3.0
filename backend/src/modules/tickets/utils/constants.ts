...
export const TICKET_COOLDOWN_MS = 10 * 60 * 1000;
export const TICKET_CREATION_LOCK_MS = 8000;
...
export const TICKET_TAGS: TicketTag[] = [
  {
...
export function getTicketColor(threadId: string | number): number {
  let hash = 0;
...
export const ERROR_MESSAGES = {
  ...
  ON_COOLDOWN: (readyAt: number) => `You recently closed a ticket. You can open another <t:${readyAt}:R>.`,
  ...
  CANNOT_REMOVE_STAFF: (userId: string) => `Cannot remove <@${userId}> — they are support staff, an admin, or the server owner.`,

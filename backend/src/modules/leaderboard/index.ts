...
import type { RedisClient, ModuleDefinition, InteractionResult } from '../../types/index.js';
...
function parseSyncResults(raw: any[], bucket: string): SyncResult[] {
...
async function performSync(redis: RedisClient, supabase: SupabaseClient): Promise<void> {
...
async function doSync(redis: RedisClient, supabase: SupabaseClient): Promise<void> {
...
async function resetBucket(redis: RedisClient, bucket: string): Promise<number> {
...
async function doResetBucket(redis: RedisClient, bucket: string): Promise<number> {
  const rawClient = redis.getClient();
...
function onMessage(redis: RedisClient, message: Message): void {
...
async function validateRpcContract(supabase: SupabaseClient): Promise<boolean> {
  for (const bucket of BUCKETS) {
    try {
...
async function recoverSyncingSets(redis: RedisClient): Promise<void> {
  const rawClient = redis.getClient();
  if (!rawClient) return;
...
async function bootstrap(redis: RedisClient, discordClient: Client, supabase: SupabaseClient): Promise<void> {
  ...
  for (const key of ['leaderboard:msg:header', ...BUCKETS.map((b) => `leaderboard:msg:${b}`)]) {
    ...
    if (msgId) {
      try {
        ...
        await redis.del(key).catch((err: any) => logger.debug({ err, key }, 'Cleanup of stale leaderboard msg key failed'));
      }
    }
  }

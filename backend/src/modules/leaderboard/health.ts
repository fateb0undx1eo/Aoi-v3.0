...
export interface LeaderboardHealthResult {
  redis: string;
  sync_worker: string;
  reset_worker: string;
  ...
  sync_staleness_ms: number | null;
  leaderboard_staleness_ms: number | null;
  reset_staleness_ms: number | null;
  instance_id: string;
  last_sync_age_ms?: number;
}

export async function getLeaderboardHealth(redis: RedisClient): Promise<LeaderboardHealthResult> {
  const result: LeaderboardHealthResult = {
    redis: 'unknown',
    sync_worker: 'unknown',
    ...
    instance_id: process.env.HOSTNAME || String(process.pid)
  };

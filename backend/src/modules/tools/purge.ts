...
export const activePurges = new Set<string>();
...
export function checkUserPermissions(member: any, guild: any): InteractionResult | null {
...
export function checkBotPermissions(guild: any, channel: any): InteractionResult | null {
...
export function validateChannel(channel: any, guild: any): InteractionResult | null {
...
export async function runPurge({ channel, count, dryRun, interaction }: PurgeParams): Promise<void> {
    ...
    while (!done) {
      ...
      for (const message of batch.values()) {
        ...
        if (dryRun) {
          ...
          if (now - lastProgressUpdate > PROGRESS_INTERVAL) {
            ...
            await safeEditReply(interaction, buildProgressText({
              channelName: channel.name, scanned, deleted, wouldDelete, failed,
              elapsed: formatElapsed(startTime), mode: count !== null ? 'count' : 'all',
              targetCount: count, status: 'Dry run mode active. Counting messages that would be deleted.'
            }));
          }
          continue;
        }
        ...
        try {
          ...
          if (deleteError.code === 10008) {
            ...
            logger.warn('Purge: channel unavailable', {
              channelId: channel.id, code: deleteError.code
            });
            done = true;
            break;
          } else {
            ...
            logger.warn('Purge: delete failed', {
              messageId: message.id, code: deleteError.code, message: deleteError.message
            });
          }
        }
        ...
        if (now - lastProgressUpdate > PROGRESS_INTERVAL) {
          lastProgressUpdate = now;
          await safeEditReply(interaction, buildProgressText({
            channelName: channel.name, scanned, deleted, failed,
            elapsed: formatElapsed(startTime), mode: count !== null ? 'count' : 'all',
            targetCount: count, status: `Processing... (${deleted} deleted so far)`
          }));
        }
      }
      ...
      await safeEditReply(interaction, buildProgressText({
        channelName: channel.name, scanned, deleted, wouldDelete, failed,
        elapsed: formatElapsed(startTime), mode: count !== null ? 'count' : 'all',
        targetCount: count, status: 'Batch complete. Fetching more messages...'
      }));
  ...
  logger.info('Purge completed', {
    channelId: channel.id, scanned, deleted, wouldDelete, failed, dryRun,
    duration: formatElapsed(startTime), avgSpeed
  });
}
